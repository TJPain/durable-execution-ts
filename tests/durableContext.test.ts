...
import { DurableContext } from "../src/durableContext";
...
describe("DurableContext.run", () => {
  it("executes fn and persists output on first run", async () => {
    const ctx = new DurableContext(taskId);
  ...
  it("returns stored output without calling fn on replay", async () => {
    await sql`INSERT INTO durable_events (task_id, event_id, output) VALUES (${taskId}, 0, ${sql.json("cached")})`;
  ...
  it("executes subsequent steps after a partially completed run", async () => {
    await sql`INSERT INTO durable_events (task_id, event_id, output) VALUES (${taskId}, 0, ${sql.json("step-0-output")})`;
    ...
    const [row] = await sql`SELECT output FROM durable_events WHERE task_id = ${taskId} AND event_id = 1`;
